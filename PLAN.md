...
- Migration debt inside Phase 3:
  - route-level reply continuation generation, parse-only prompts, reply artifact shaping, and reply surface planning now flow through `apps/web/lib/agent-v2/orchestrator/replyContinuationPlanner.ts`, while `apps/web/app/api/creator/v2/chat/route.reply.ts` remains the route-boundary wrapper for reply preflight parsing/default resolution and handled-reply persistence/finalization
