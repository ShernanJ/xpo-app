...
- Remaining work:
  ...
  - reply continuation generation now lives in `apps/web/lib/agent-v2/orchestrator/replyContinuationPlanner.ts`, while `apps/web/app/api/creator/v2/chat/route.reply.ts` still owns the route-boundary preflight parse/default wrapper, parse prompts, and artifact shaping and must be reconciled fully with the runtime capability boundary
