...
- Reply preflight parsing/default resolution and handled-reply persistence/finalization now flow through `apps/web/app/api/creator/v2/chat/route.reply.ts`, while parse-only prompts, reply artifact shaping, and reply surface planning now flow through `apps/web/lib/agent-v2/orchestrator/replyContinuationPlanner.ts`.
